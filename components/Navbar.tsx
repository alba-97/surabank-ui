...
export default function Navbar({ active, onTransfer }: NavbarProps) {
  ...
  return (
    <motion.div
      className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[414px] bg-surface rounded-t-3xl shadow-[0_-8px_30px_0_rgba(0,0,0,0.06)] dark:shadow-[0_-8px_30px_0_rgba(0,0,0,0.4)] px-10 py-4 flex items-center justify-between"
      initial={{ y: 100 }}
      animate={{ y: 0 }}
      transition={{ duration: 0.5, delay: 0.4 }}
    >
      <motion.button
        className={`flex flex-col items-center gap-1 cursor-pointer ${active === 'home' ? 'text-accent' : 'text-fg'}`}
        whileTap={{ scale: 0.85 }}
        onClick={() => {
          playTap();
          router.push('/home');
      ...
      <motion.button
        className="flex flex-col items-center gap-1 cursor-pointer text-fg"
        whileTap={{ scale: 0.85 }}
        onClick={() => {
          playTap();
          onTransfer?.();
      ...
      <motion.button
        className={`flex flex-col items-center gap-1 cursor-pointer ${active === 'card' ? 'text-accent' : 'text-fg'}`}
        whileTap={{ scale: 0.85 }}
        onClick={() => {
          playTap();
          router.push('/cards');
